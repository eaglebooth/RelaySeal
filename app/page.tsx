...
const actions: Action[] = [
  { method: "register_service", eyebrow: "01 / Establish", title: "Register service", actor: "Service controller", fields: [
    { key: "service_id", label: "Service ID", placeholder: "payments-api" }, { key: "active_operator", label: "Current operator", placeholder: "0x…" }, { key: "repository", label: "Evidence repository", placeholder: "org/repo" },
  ]},
  { method: "create_policy", eyebrow: "02 / Bind", title: "Draft transfer policy", actor: "Service controller", fields: [
    { key: "controller", label: "Controller", placeholder: "0x…" }, { key: "service_id", label: "Service ID", placeholder: "payments-api" }, { key: "policy_id", label: "Policy ID", placeholder: "payments-r7" }, { key: "incoming_operator", label: "Incoming operator", placeholder: "0x…" }, { key: "corroboration_repository", label: "Independent repository", placeholder: "independent-org/audit-repo" }, { key: "revision", label: "Service revision", placeholder: "1", type: "number" }, { key: "requirements", label: "Required evidence sections", placeholder: "Require deployment, incidents, risks, rollback, actions, owners, deadlines and acknowledgement." },
  ]},
  { method: "approve_policy", eyebrow: "03 / Co-sign", title: "Approve exact policy", actor: "Outgoing + incoming", fields: [
    { key: "controller", label: "Controller", placeholder: "0x…" }, { key: "service_id", label: "Service ID", placeholder: "payments-api" }, { key: "policy_id", label: "Policy ID", placeholder: "payments-r7" }, { key: "expected_digest", label: "Policy digest", placeholder: "64 hex characters" },
  ]},
  { method: "submit_handover", eyebrow: "04 / Commit", title: "Submit evidence commitment", actor: "Active outgoing operator", fields: [
    { key: "controller", label: "Controller", placeholder: "0x…" }, { key: "service_id", label: "Service ID", placeholder: "payments-api" }, { key: "policy_id", label: "Policy ID", placeholder: "payments-r7" }, { key: "handover_id", label: "Handover ID", placeholder: "shift-2026-09-18" }, { key: "evidence_url", label: "Commit-pinned raw Markdown URL", placeholder: "https://raw.githubusercontent.com/org/repo/<40-hex-commit>/handover.md" }, { key: "evidence_sha256", label: "SHA-256", placeholder: "64 hex characters" }, { key: "evidence_bytes", label: "Exact byte length", placeholder: "2048", type: "number" }, { key: "nonce", label: "One-time nonce", placeholder: "shift-2026-09-18-a" },
  ]},
  { method: "submit_corroboration", eyebrow: "05 / Corroborate", title: "Submit independent corroboration", actor: "Bound incoming operator", fields: [
    { key: "controller", label: "Controller", placeholder: "0x…" }, { key: "service_id", label: "Service ID", placeholder: "payments-api" }, { key: "policy_id", label: "Policy ID", placeholder: "payments-r7" }, { key: "handover_id", label: "Handover ID", placeholder: "shift-2026-09-18" }, { key: "evidence_url", label: "Independent commit-pinned URL", placeholder: "https://raw.githubusercontent.com/independent-org/audit-repo/<40-hex-commit>/audit.md" }, { key: "evidence_sha256", label: "SHA-256", placeholder: "64 hex characters" }, { key: "evidence_bytes", label: "Exact byte length", placeholder: "2048", type: "number" }, { key: "nonce", label: "One-time nonce", placeholder: "audit-2026-09-18-a" },
  ]},
  { method: "assess_handover", eyebrow: "06 / Judge", title: "Assess both sources", actor: "Any caller · consensus decides", fields: [{ key: "controller", label: "Controller", placeholder: "0x…" }, { key: "service_id", label: "Service ID", placeholder: "payments-api" }, { key: "policy_id", label: "Policy ID", placeholder: "payments-r7" }, { key: "handover_id", label: "Handover ID", placeholder: "shift-2026-09-18" }] },
  { method: "accept_handover", eyebrow: "07 / Acknowledge", title: "Accept exact handover", actor: "Bound incoming operator", fields: [{ key: "controller", label: "Controller", placeholder: "0x…" }, { key: "service_id", label: "Service ID", placeholder: "payments-api" }, { key: "policy_id", label: "Policy ID", placeholder: "payments-r7" }, { key: "handover_id", label: "Handover ID", placeholder: "shift-2026-09-18" }, { key: "expected_handover_digest", label: "Handover digest", placeholder: "64 hex characters" }] },
  { method: "activate_handover", eyebrow: "08 / Transfer", title: "Activate authority", actor: "Service controller", fields: [{ key: "controller", label: "Controller", placeholder: "0x…" }, { key: "service_id", label: "Service ID", placeholder: "payments-api" }, { key: "policy_id", label: "Policy ID", placeholder: "payments-r7" }, { key: "handover_id", label: "Handover ID", placeholder: "shift-2026-09-18" }] },
  { method: "perform_guarded_operation", eyebrow: "09 / Prove", title: "Run guarded operation", actor: "New active operator", fields: [{ key: "controller", label: "Controller", placeholder: "0x…" }, { key: "service_id", label: "Service ID", placeholder: "payments-api" }, { key: "operation_id", label: "Operation ID", placeholder: "deploy-2026-09-18" }, { key: "payload_digest", label: "Payload digest", placeholder: "64 hex characters" }] },
];

const readFields: Record<string, Field[]> = {
  get_service: [{ key: "controller", label: "Controller", placeholder: "0x…" }, { key: "service_id", label: "Service ID", placeholder: "payments-api" }],
  get_policy: [{ key: "controller", label: "Controller", placeholder: "0x…" }, { key: "service_id", label: "Service ID", placeholder: "payments-api" }, { key: "policy_id", label: "Policy ID", placeholder: "payments-r7" }],
  get_handover: [{ key: "controller", label: "Controller", placeholder: "0x…" }, { key: "service_id", label: "Service ID", placeholder: "payments-api" }, { key: "policy_id", label: "Policy ID", placeholder: "payments-r7" }, { key: "handover_id", label: "Handover ID", placeholder: "shift-2026-09-18" }],
  get_operation_receipt: [{ key: "controller", label: "Controller", placeholder: "0x…" }, { key: "service_id", label: "Service ID", placeholder: "payments-api" }, { key: "operation_id", label: "Operation ID", placeholder: "deploy-001" }], get_stats: [],
};
const reads = Object.keys(readFields);
...
export default function Home() {
  ...
  const errors = useMemo(() => validateValues(action.fields, values), [action, values]);
  ...
  async function submit() {
    if (!valid) { setPhase("INVALID_INPUT"); setMessage("Correct the highlighted fields before signing."); return; }
    ...
    const interrupted = !result.success && /Tracking timed out/.test(result.error || "");
    setPhase(result.success ? "FINALIZED" : interrupted ? "VERIFY_ON_EXPLORER" : "FAILED");
  ...
  async function loadReadback() {
    ...
    const result = await readContract(readMethod, readValues);
  ...
  return <main>
    ...
    <section className="desk" id="desk">
      ...
      <div className="desk-grid">
        ...
        <div className="form-sheet"><p>{action.eyebrow}</p><h3>{action.title}</h3><div className="actor"><ShieldCheck size={17}/><span>Authorized actor</span><b>{action.actor}</b></div><div className="fields">{action.fields.map((field) => <label key={field.key}><span>{field.label}</span><input className={errors[field.key] ? "invalid" : ""} type={field.type || "text"} placeholder={field.placeholder} value={values[field.key] || ""} onChange={(e) => setValues({ ...values, [field.key]: e.target.value })}/>{values[field.key] && errors[field.key] && <small className="field-error">{errors[field.key]}</small>}</label>)}</div><button className="submit" disabled={!ready || !wallet || !valid || phase === "SUBMITTED" || phase === "PENDING"} onClick={submit}>{ready ? (wallet ? (valid ? `Submit ${action.eyebrow.slice(0, 2)}` : "Correct invalid fields") : "Connect wallet to continue") : "Deploy contract to enable writes"}<ArrowRight size={17}/></button></div>
        <div className="read-sheet"><p>CANONICAL READBACK</p><h3>Accepted state</h3><label><span>Record type</span><select value={readMethod} onChange={(e) => { setReadMethod(e.target.value); setReadValues({}); }}>{reads.map((r) => <option key={r}>{r}</option>)}</select></label>{readFields[readMethod].map((field) => <label key={field.key}><span>{field.label}</span><input value={readValues[field.key] || ""} onChange={(e) => setReadValues({...readValues, [field.key]: e.target.value})} placeholder={field.placeholder}/></label>)}<button className="outline" onClick={loadReadback}>Refresh accepted state</button><pre>{readback}</pre></div>
      </div>
